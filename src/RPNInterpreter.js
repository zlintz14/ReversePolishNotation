...
var main = function () {
    ...
    var RPN = prompt("Enter you function is Reverse Polish Notation (use *,/,+,-,^ for operators): ");
    ...
    if (!checkIsInRPN(RPN) && !determineNoSpaceDoubleDigitInput(RPN) && !determineNoSpaceSingleDigitInput(RPN)) {
        ...
        if (interpreted != "") {
            var parenthesis = "";
            for (var i = 0; i < parenthesisCount; i++) {
                parenthesis += "(";
            }
            console.log("Your function in Infix Notation is: " + parenthesis + interpreted + " = " + total);
        }
        var response = prompt("Would you like to enter another equation? Enter yes or no: ");
        if (response === "yes") {
            main();
        }
    }
...
var checkIsInRPN = function (input) {
    var count = 0;
    //find first number
    while (input[count] === " ") {
        count++;
...
var determineNoSpaceSingleDigitInput = function (input) {
    var termCount = 0;
    var operatorCount = 0;
    for (var i = 0; i < input.length; i++) {
...
var determineNoSpaceDoubleDigitInput = function (input) {
    var temp = "";
    var count1 = 0;
    //collect number
